/**
 * SHINERVA - Admin Diagnostic Route (Test Magic Link)
 * ==================================================
 * POST /api/admin/test-magic-link
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const apiRes = await fetch(fetchUrl, {
      ...
      body: JSON.stringify({ email, action: 'signIn', continueUrl: targetOrigin })
    });
    ...
    if (apiRes.ok) {
      return res.status(200).json({
        diagnostics: {
          test: 'Magic Link SMTP Cascade',
          targetUrl: targetOrigin,
          action_requested: 'signIn',
